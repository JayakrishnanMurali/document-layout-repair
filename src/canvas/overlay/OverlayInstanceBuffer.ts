...
export class OverlayInstanceBuffer {
  ...
  private pack(
    ...
    for (const pageIndex of visiblePageIndexes) {
      upperBoundInstanceCount += getPageNodeCount(document, pageIndex);
    }
  ...
  private writeInstance(
    ...
    const style =
      NODE_CLASS_STYLES[getLayoutNodeClassName(geometry.classIds[nodeId])];
    const isLowConfidence =
      (geometry.flags[nodeId] & NODE_FLAG_LOW_CONFIDENCE) !== 0;
    const borderColor = isLowConfidence
      ? LOW_CONFIDENCE_BORDER_COLOR
      : style.borderColor;
    const baseFillColor = isLowConfidence
      ? LOW_CONFIDENCE_FILL_COLOR
      : style.fillColor;
    ...
    const isSolidTint =
      Math.min(
        height * devicePixelsPerWorldUnit,
        width * devicePixelsPerWorldUnit,
    ...
    const fillAlpha = isSolidTint
      ? Math.round(borderColor[3] * 0.7)
      : baseFillColor[3];
    ...
    this.byteView[byteBase + INSTANCE_BORDER_COLOR_BYTE_OFFSET] =
      borderColor[0];
    this.byteView[byteBase + INSTANCE_BORDER_COLOR_BYTE_OFFSET + 1] =
      borderColor[1];
    this.byteView[byteBase + INSTANCE_BORDER_COLOR_BYTE_OFFSET + 2] =
      borderColor[2];
    this.byteView[byteBase + INSTANCE_BORDER_COLOR_BYTE_OFFSET + 3] =
      borderColor[3];
